...
### Bootstrap Methods
The ultimate goal of the methods outlined in bootstrap are to take observations, $\bb{X}$, from an unknown distribution, $F$, and understand the sampling distribution of a chosen random quantity, $R(\bb{X}, F)$, of interest using only the observed data you have available. In the context of this paper, there existed a method called "Jackknife" which mainly worked for two forms of the $R(\bb{X}, F)$ that is
\begin{align*}
R(\bb{X}, F) &= t(\bb{X}) - \theta(F)\\
R(\bb{X}, F) &= \frac{t(\bb{X}) - \widehat{Bias}(t) - \theta(F)}{\sqrt{\widehat{Var}(t)}}
\end{align*}
where $t(\bb{X})$ estimates $\theta(F)$. What is worth noting is that jackknife approximates the distribution of $R$ by sampling $n-1$ times **without** replacement. Bootstrap is then introduced and follows the general framework below
...
2. Sample from $\bb{X}$ **with** replacement to get $\bb{X}^*$  which has data points $\{x_1^*, ..., x_n^*\} \subseteq \bb{X}$. Do this multiple times since each $\bb{X_i^*}$ will be from the sampling distribution $\hat{F}$.
3. Now we get the _bootstrap distribution_ which is the "empirical distribution" for $R(\bb{X},F)$, namely $R^* = R(\bb{X^*}, \hat F)$.
...
Now, we can show the following probability statement <!--- finish --->
\begin{align*}
P(R^* = x_{(l)} - x_{(m)}) &= P(X_{(m)}^* - x_{(m)} = x_{(l)} - x_{(m)}) \\
&= P(X_{(m)}^* = x_{(l)})\\
&= P(X_{(m)}^* > x_{(l)}) - P(X_{(m)}^* > x_{(l+1)})\\
&= P\left(Bin\left(n,\frac{l-1}{n}\right) \leq m-1\right) - P\left(Bin\left(n,\frac{l}{n}\right) \leq m-1\right)
\end{align*}

Using the probability distribution derived above, we can produce the following table as follows. Let $n=13$ and $m=7$ 
```{r}
m = matrix(nrow=2,ncol=6)
for(i in 1:6) {
  m[1,i] = pbinom(6, 13, (i)/13) - pbinom(6,13, (i+1)/13)
  m[2,i] = pbinom(6, 13, (13-i-1)/13) - pbinom(6,13, (13-i)/13)
}
...
by the following
$$E_*(R^*)^2 = \sum_{l=1}^{13} [x_{(l)} - x_{(7)}]^2 \cdot P(R^* = x_{(l)} - x_{(7)})$$
because we have an estimated discrete probability distribution now. It is worth noting that when the density of $F$, $f(x)$, is bounded and continuous, then
$$nE_*(R^{*^2}) \to \frac{1}{4}f^2(\theta) \quad \text{ as } \quad n\to \infty$$
where $f(\theta)$ is defined as computing the density of $F$ at the median $\theta(F)$.

Next, we will consider an example using a symmetric probability distribution. We will define $\hat F_{SYM}$ by being symmetric about the median $x_{(m)}$ and having the probability mass of $\frac{1}{2n-1}$ for each of the $2n-1$ support value, namely $x_{(1)}, ..., x_{(n)}, 2x_{(m)}-x_{(1)}, ..., 2x_{(m)} - x_{(n)}$. We do this so that all $2n-1$ sample points are symmetric about $x_{(m)}$. Call $z$ the bootstrap sample elements with ordering $z_{(1)} \leq ... \leq z_{(2n-1)}$. Then, using the same derivation of the probability mass function for $R^*$, let $l \in [1,2n-1]$
$$P_*(R^* = z_{(l)} - x_{(m)}) = P\left(Bin\left(n,\frac{l-1}{2n-1}\right) \leq m-1\right) - P\left(Bin\left(n,\frac{l}{2n-1}\right) \leq m-1\right)$$
It is worth noting that we are only considering a sample size of $n$ because the distribution we are sampling from is symmetric, therefore, there exists only $n$ values that we can sample the bootstrap from, but there exists a total of $2n-1$ possible "indices" from which to sample from.

Again, suppose $n=13$, then we can replicate the table as follows
```{r}
m = matrix(nrow=1,ncol=10)
for(i in 1:10) {
  m[1,i] = pbinom(6, 13, (i+2)/25) - pbinom(6,13, (i+3)/25)
}
w = matrix(data=m, nrow=2,byrow = T)
print(round(w, digits=4))
```

Now, we transition into a different way of doing the bootstrap sampling. Rather than taking random samples from $\bb{X} = (x_1,...,x_n)$, we can take
$$X_i^* = \bar x + c[x_{I_i} - \bar x + \hat\sigma Z_i]$$
where $I_i$ is a random index chosen indepedently from $\{1,2,...,n\}$ and $Z_i$ is a predetermined random variable with mean 0 and variance denoted by $\sigma_Z^2$. Then, $\bar x, \hat \sigma,$ and $c$ are chosen such that $E(X_i^*) = \bar x$ and $Var(X_i^*) = \hat \sigma^2$. The motivation behind this new sampling technique is to not rely on strict assumptions on the functional form of the distribution. In other words, assuming symmetry (like before) is very strict. Not only does the sampling procedure differ, but the manner to check how reliable this sampling procedure changes. We now switch from investigating squared error loss to absolute error loss with the random variable given by
$$R(\bb{X}, F) = \frac{|t(\bb{X}) - \theta(F)|}{\sigma(F)} \implies R^* = \frac{|t(\bb{X^*}) - \theta(\hat F)|}{\hat\sigma}$$
You may be asking why the change? This is because we want to gain more numerical stability in our calculation. We are still studying the true median $\theta(F)$, however, now the bootstrapped quantity is the absolute error relative to population standard deviation $\sigma(F)$ which lets $R^*$ be invariant to any scaled changes in $\hat \sigma$ estimating the true standard deviation.

How do we actually test if this new bootstrap quantity is worth studying? We will do Monte Carlo simulations to test this in the following example. This example follows a similar structure to the one in the paper, however, we believe we have made the motivation for this method a little clearer in our personal example. Let $X_i \overset{iid}{\sim} N(0,1)$ for $i = 1,...,13$. Then, we know that 
$$E_F(R) = E_F(X_{(m)} - 0) = 0$$
We will compare the results from the first two unsmooth bootstrap examples for the simple and symmetric cases with the smooth bootstrap method that scales with the standard devation. We begin with the two unsmooth bootstrap examples with the goal being that $E(R^*)$ is close to 0 with some small standard deviation.
```{r}
set.seed(100)
n = 13
...
print(paste0("SD: ", round(sd(exp_val), digits = 3)))
```
We note that for the 10 trials, we get $E(R^*) \approx 0$ with a very small standard deviation. This is reassuring of the theory described above; however, it is worth noting that no bootstrapped samples were needed in order to get those expectations. We derived the probability distributions by hand and used theory to make those calculations work. Thus, in the most simplest of cases, we already get results close to the truth. However, the researchers point out that for the example we are using, the normal distribution is smooth, well behaved, symmetric, and centered at 0. All of these properties would allow for these two simple, unsmoothed bootstrap approaches to do well. We especially see this to be true in the symmetric method because we see that all 10 trials resulted in expected values that were basically 0 and the standard deviation rounded to 0. Thus, correctly assuming symmetry drastically improved the results.

Next we will explore the Smoothed Bootstrap approach described right before this example for the same data as before. The authors argue that in settings where the simulated distributions are not as well behaved, the smoothing procedure may be better, yet that is still unclear and more research needs to be done to confirm that hypothesis. Regardless, we will still let $X_i \overset{iid}{\sim} N(0,1)$ and we will let $Z_i \sim U\left(\frac{-d}{2}, \frac{d}{2}\right)$ for $d \in \{0,0.25,0.5,1\}$. We will use $N=50$ Monte Carlo replications for the 10 trials above. Addionally let $c = (1 + \sigma_Z^2)^{-1/2}$. With this setup, we get the following results
```{r}
...
Therefore, despite the smoothing condition, the estimates for the expected value are further from the truth than the first two examples, while the standard deviations are still small. Again, the researchers claim that if the distribution of $R$ itself was perhaps less smooth than was simulated in this exercise, then it would perform better; however, this paper provides no strict evidence to support that.
