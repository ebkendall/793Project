...
2. Sample from $\bb{X}$ **with** replacement to get $\bb{X}^*$  which has data points $\{x_1^*, ..., x_n^*\} \subseteq \bb{X}$. Do this multiple times since each $\bb{X_i^*}$ will be from the sampling distribution $\widehat{F}$.
3. Now we get the _bootstrap distribution_ which is the "empirical distribution" for $R(\bb{X},F)$, namely $R^* = R(\bb{X^*}, \widehat F)$.

The whole motivation is that if the resampled distribution $\widehat F$ is remotely close to the true $F$, then the estimator for $R$'s distribution will be close to the truth. A large factor in the success of this method, however, is the functional form of $R$. Additionally, while it may be relatively straightforward to calculate the expected value and variance of $R^*$, actually deriving its distribution proves to be difficult. In this paper, we will dive into the three main derivation techniques for uncovering this distribution. They are
...
where $x_{(m)}$ is the observed median. Next, we investigate the probabilities associated with $R^*$ as it is important in getting the random variable's expected value and other summaries. First, let $l \in \{1, 2, \ldots, n-1\}$. Then
\begin{align*}
\Pr(X^*_{(m)} > x_{(l)}) &= \Pr(N_{(1)}^* + ...+ N_{(l)}^* < m)
\end{align*}
Since each $N_{(i)}^*$ can take a value between 0 and $n$ with equal probability and each $N_{(i)}^*$ is assumed independent, then we know that $\sum_{i=1}^l N_{(i)}^* \sim \operatorname{Binom}\left(n, \frac{l}{n}\right)$. Therefore, call $W^* = \sum_{i=1}^l N_{(i)}^*$ and note that since we are in a discrete setting, we can rewrite the probability statement as
\begin{align*}
...
You may be asking why the change? This is because we want to gain more numerical stability in our calculation. We are still studying the true median $\theta(F)$, however, now the bootstrapped quantity is the absolute error relative to population standard deviation $\sigma(F)$ which lets $R^*$ be invariant to any scaled changes in $\widehat \sigma$ estimating the true standard deviation.
