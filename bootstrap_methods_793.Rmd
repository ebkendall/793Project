...
plot(ns, cov_diff, xlab="n", ylab="Relative Error", main="Relative Error in Covariance Estimate vs Sample Size")
```
```{r}
set.seed(0)
...
plot(ns, cov_diff, xlab="n", ylab="Relative Error", main="Relative Error in Covariance Estimate vs Sample Size")
```
Both bootstrapping by residuals and bootstrapping by pairs have some issues estimating covariances regardless of the sample size; the outliers in bootstrapping by pairs tend to be smaller in magnitude, however. In general though, these covariance estimates are once again unbiased, demonstrating that bootstrap estimates of variances is still valid for vector parameters $\beta$. For simplicity, we will focus on scalar parameters $\beta$ for the rest of the post. 
