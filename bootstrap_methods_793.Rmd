...
How do we actually test if this new bootstrap quantity is worth studying? We will do Monte Carlo simulations to test this in the following example. Let $X_i \overset{iid}{\sim} N(0,1)$ for $i = 1,...,13$. Then, we know that 
$$E_F(R) = 0.95$$
We will compare the results from the first two unsmooth bootstrap examples for the simple and symmetric cases with the smooth bootstrap method that scales with the standard devation. We will apply Monte Carlo simulations with the smoothed bootstrap method with a resampled size of $N=50$. We will let $Z_i\sim U\left(\frac{-d}{2}, \frac{d}{2}\right)$ for $d \in \{0,0.25,0.5,1\}$. 
The results of the simulation indicate that with a bootstrapped sample size of $N=50$ and 10 trials, the simplest boostrap procedure results in an estimated mean of 1.21
with a standard deviation of 0.54, the symmetric method results in a mean of 0.26 with a standard deviation of 0.17, and the other smooth bootstrap methods had a mean that was consistently about 1.15 with a standard deviation of 0.23. Therefore, even with the strong symmetry assumption for the second bootstrap procedure, it still produces results that rival those of the more numerically stable procedures which may take a little more computation and thought to produce. The authors argue that in settings where the simulated distributions are not as well behaved, the smoothing procedure may be better, yet that is still unclear and more research needs to be done to confirm that hypothesis. (The code and results for the first procedure are given below.)
```{r}
set.seed(100)
n = 13
...
print("Trails 1-10"); round(est_exp[,1], digits = 2)
print(paste0("Average: ", round(mean(est_exp[,1]), digits = 2)))
print(paste0("SD: ", round(sd(est_exp[,1]), digits = 2)))
```
