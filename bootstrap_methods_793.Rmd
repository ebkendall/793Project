...
1. Given the existing data $\bb{X} = (x_1,...,x_n)$, assign each data point probability $1/n$ such that selecting an observation from the sample has equal probability. We denote this emperical sampling distribution $\widehat{F}$.
2. Sample from \widehat{F} (or equivalently, from $\bb{X}$ **with** replacement) to get $\bb{X}^* = (x_1^*, ..., x_n^*)$, where each $x_i^* \in \bb{X}$.
3. Performing the resampling of step 2 multiple times to yield several $\bb{X_i^*}$ from the sampling distribution $\widehat{F}$. By computing $R^*_i = R(\bb{X^*_i}, \widehat F)$ from our bootstrap samples, we thus get the _bootstrap distribution_, which is a sort of "empirical distribution" for the quantity of interest $R(\bb{X},F)$.
...
### A Direct Theoretical Calculation
...
Again, supposing $n=13$, we can replicate the table as follows:
```{r}
...
where $I_i$ is a random index chosen indepedently from $\{1,2,...,n\}$ and $Z_i$ is a predetermined random variable with mean 0 and variance denoted by $\sigma_Z^2$. Then, $\bar x, \widehat \sigma,$ and $c$ are chosen such that $\E(X_i^*) = \bar x$ and $\var(X_i^*) = \widehat \sigma^2$. The motivation behind this new sampling technique is to not rely on strict assumptions on the functional form of the distribution. In other words, assuming symmetry (like before) is very strict, but this new technique allows us to relax this assumption. Not only does the sampling procedure differ, but the manner to check how reliable this sampling procedure changes as well: We now switch from investigating squared error loss to absolute error loss with the random variable given by
...
$$\E_F[R] = \E_F[X_{(m)} - 0] = 0.$$
We will compare the results from the first two non-smooth bootstrap examples for the simple and symmetric cases with the smooth bootstrap method that scales with the standard devation. We begin with the two non-smooth bootstrap examples, with the goal that $E[R^*]$ be close to 0 with some small standard deviation.
```{r}
set.seed(100)
n = 13
