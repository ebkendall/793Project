...
3. Performing the resampling of step 2 multiple times to yield several $\bb{X_i^*}$ from $\widehat{F}$. By computing $R^*_i = R(\bb{X^*_i}, \widehat F)$ from our bootstrap samples, we thus get the _bootstrap distribution_, which is a sort of "empirical distribution" for the quantity of interest $R(\bb{X},F)$.
